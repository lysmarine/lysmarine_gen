...
if [ $LMARCH == 'arm64' ]; then
  pip3 install pywavefront pyglet gps gevent-websocket importlib.metadata "python-socketio<5" "flask-socketio<5"
else
  apt-get install -y -q python3-flask-socketio
  pip3 install pywavefront pyglet gps gevent-websocket importlib.metadata "python-socketio<5"
fi
